    ...
    <div className="font-notokr p-6">
      ...
      <div className="p-6 space-y-4 w-full max-w-4xl mx-auto">
        ...
        {cartItems.map((item, idx) => (
          <div key={idx} className="h-40 flex items-center justify-around border-y border-y-gray-400">
            <input className="h-5" type="checkbox" />
            <img className="h-30" src={`http://localhost:8080/${encodeURIComponent(
                    item.images)}`} alt="상품 이미지" />
            <span>{item.name.length > 13
